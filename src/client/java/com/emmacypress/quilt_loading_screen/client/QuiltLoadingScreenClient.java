...
public class QuiltLoadingScreenClient {
	...
	public void renderPatches(GuiGraphics graphics, float delta, boolean ending, float alpha) {
		// spike prevention
		// delta 变为了帧间毫秒差
		if (delta < 100.0f){
		...
		for (FallingPatch patch : fallingPatches) {
			patch.render(graphics, minecraft.options.darkMojangStudiosBackground().get(), alpha);
		}
	}
	...
	private class FallingPatch {
		...
		public void render(GuiGraphics graphics, boolean monochrome, float alpha) {
			Matrix3x2fStack matrices = graphics.pose();
			matrices.pushMatrix();
			matrices.translate((float) x, (float) y);

			matrices.rotate((float) rot);
